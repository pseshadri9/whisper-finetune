...
"""
Code blocks for "High Resolution Piano Transcription with Pedals"
See https://github.com/bytedance/piano_transcription/blob/master/pytorch/models.py
"""
...
class RegressOnsetOffsetFrameVelocityCRNN(nn.Module):
    def __init__(self, classes_num, mel_bins, midfeat, momentum):
        super().__init__()
...
class RegressPedalCRNN(nn.Module):
    def __init__(self, classes_num, mel_bins, midfeat, momentum):
        ...
        self.reg_pedal_onset_model = AcousticModelCRnn8Dropout(classes_num, midfeat, momentum)
        self.reg_pedal_offset_model = AcousticModelCRnn8Dropout(classes_num, midfeat, momentum)
        self.reg_pedal_frame_model = AcousticModelCRnn8Dropout(classes_num, midfeat, momentum)
...
class NotePedal(nn.Module):
    def __init__(self, frames_per_second, classes_num):
        ...
        # Hard-coded values to load pre-trained models from bytedance repo
        self.note_model = RegressOnsetOffsetFrameVelocityCRNN(classes_num, 229, 1792, 0.01)
        self.pedal_model = RegressPedalCRNN(1, 229, 1792, 0.01)
