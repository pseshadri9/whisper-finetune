...
def preprocess(
    ...
    audio_files = os.path.join(dir_name, "audio.csv")
    label_files = os.path.join(dir_name, "labels.csv")
    generate_cache_file_list(dir_name, processed_ext, filename=audio_files)
...
def preprocess_audio(
    ...
    for root, dirs, files in pbar:
        ...
        for idx, x in enumerate(audio_files):
            ...
            with open(f"{os.path.join(destination_dir, prefix, x[:-len(audio_ext)])}{processed_ext}", "wb") as f:
                np.save(f, waveform)
...
def generate_data_splits_file(
    audio_files_csv: str, label_files_csv: str, proportion: list[float] = [0.8, 0.1, 0.1]
) -> str:
    audio_files = np.loadtxt(audio_files_csv, dtype=str)
    labels = np.loadtxt(label_files_csv, dtype=str)
